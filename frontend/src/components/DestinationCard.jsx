...
import { Calendar, MapPin, Plus, Bed, Car, Activity } from "lucide-react"
  ...
  return (
    <div className="bg-white rounded-lg shadow-md p-6">
      <div className="flex justify-between items-start mb-4">
        <div>
          <h3 className="text-2xl font-bold text-gray-900 mb-2">{localDestination.name}</h3>
          ...
          <div className="space-y-3">
            {localDestination.activities && localDestination.activities.length > 0 ? (
              localDestination.activities.map((activity) => (
                <ActivityCard key={activity.id} activity={activity} type="activity" />
              ))
            ) : (
          ...
          <div className="space-y-3">
            {localDestination.accommodations && localDestination.accommodations.length > 0 ? (
              localDestination.accommodations.map((accommodation) => (
                <ActivityCard key={accommodation.id} activity={accommodation} type="accommodation" />
              ))
            ) : (
          ...
          <div className="space-y-3">
            {localDestination.transport && localDestination.transport.length > 0 ? (
              localDestination.transport.map((transport) => (
                <ActivityCard key={transport.id} activity={transport} type="transport" />
              ))
            ) : (
