	...
	try {
		const response = await api.post(
			`/destinations/${destinationId}/activities`,
			{
				...
				time: new Date(
					`${activityData.date}T${activityData.time}`
				).toISOString(),
...
export const addAccommodation = async (destinationId, accommodationData) => {
	try {
		...
		throw new Error(
			error.response?.data?.message || "Failed to add accommodation"
		);
	}
};
...
export const addTransport = async (destinationId, transportData) => {
	try {
		const response = await api.post(
			`/destinations/${destinationId}/transport`,
			{
				type: transportData.type,
				details: transportData.name,
				time: new Date(
					`${transportData.date}T${transportData.time}`
				).toISOString(),
			}
		);
		return response.data.data;
		...
		throw new Error(
			error.response?.data?.message || "Failed to add transport"
		);
	}
};
